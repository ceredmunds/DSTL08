var Game = {
  ...
  scriptShortVersion: [],//for 12 entities
  scriptLongVersion: [],//for 24 entities
  ...
  getScriptedTrials: function () {
    if (Game.conditionExperiment === 1 || Game.conditionExperiment === 4) {
      var randomShort = Game.randomIntFromInterval(0, TrialsRatio.cond1_12.length - 1);
      var randomLong = Game.randomIntFromInterval(0, TrialsRatio.cond1_24.length - 1);

      Game.scriptShortVersion = TrialsRatio.cond1_12[randomShort];
      Game.scriptLongVersion = TrialsRatio.cond1_24[randomLong];

      console.log("Version corta" + Game.scriptShortVersion)
      console.log("Version larga" + Game.scriptLongVersion)
    } else {
      var randomShort = Game.randomIntFromInterval(0, TrialsRatio.cond2_12.length - 1);
      var randomLong = Game.randomIntFromInterval(0, TrialsRatio.cond2_24.length - 1);

      Game.scriptShortVersion = TrialsRatio.cond2_12[randomShort];
      Game.scriptLongVersion = TrialsRatio.cond2_24[randomLong];
    }
  ...
  getCategoriesByCondition: function () {
    ...
    if (Game.conditionExperiment === 1 || Game.conditionExperiment === 4) {
      Game.categoriesByCondition = [Category.UNKNOWN, Category.FRIEND, Category.HOSTILE, Category.NEUTRAL]
    } else {
  ...
  animation: function () {
    ...
    createParticipant: function () {
      ...
      $.ajax({
        ...
        data: {
          ...
          trialsShortVersion: JSON.stringify(Game.scriptShortVersion),
          trialsLongVersion: JSON.stringify(Game.scriptLongVersion),
        },
    ...
    getMapping: function () {
      ...
      //TODO
      colorsControl = [Colour.BLUE, Colour.YELLOW, Colour.PURPLE];
      // colorsCategorical = [Colour.BLUE,  Colour.YELLOW , Colour.PURPLE];
      if (Game.conditionExperiment === 1) { // control_green
      ...
      } else if (Game.conditionExperiment === 4) { // control_blue
        Game.mapping['' + Category.UNKNOWN] = Colour.GRAY
        Game.mapping['' + Category.FRIEND] = Colour.BLUE
        Game.mapping['' + Category.HOSTILE] = Colour.RED
        Game.mapping['' + Category.NEUTRAL] = Colour.GREEN

        Game.mapping ['' + Category.ASSUMED_FRIEND] = Colour.BLUE
        Game.mapping ['' + Category.ASSUMED_HOSTILE] = Colour.RED
      } else if (Game.conditionExperiment === 2) { // categorical_green
      ...
      } else if (Game.conditionExperiment === 5) { // categorical_blue
        colorsControl = [Colour.YELLOW, Colour.PURPLE]
        Game.mapping['' + Category.UNKNOWN] = Colour.GRAY
        Game.mapping['' + Category.FRIEND] = Colour.BLUE
        Game.mapping['' + Category.HOSTILE] = Colour.RED
        ...
        Game.mapping ['' + Category.ASSUMED_FRIEND] = coloursCopy[0]
        Game.mapping ['' + Category.ASSUMED_HOSTILE] = coloursCopy[1]
      } else if (Game.conditionExperiment === 3) { // pictorial_green
        Game.mapping['' + Category.UNKNOWN] = Colour.GRAY
        Game.mapping['' + Category.FRIEND] = Colour.GREEN
        Game.mapping['' + Category.HOSTILE] = Colour.RED
        Game.mapping['' + Category.NEUTRAL] = Colour.BLUE
      ...
      } else if (Game.conditionExperiment === 6) { // pictorial_blue
        Game.mapping['' + Category.UNKNOWN] = Colour.GRAY
        Game.mapping['' + Category.FRIEND] = Colour.BLUE
        Game.mapping['' + Category.HOSTILE] = Colour.RED
        Game.mapping['' + Category.NEUTRAL] = Colour.GREEN
        Game.mapping['' + Category.ASSUMED_FRIEND] = Colour.C_BLUE
        Game.mapping['' + Category.ASSUMED_HOSTILE] = Colour.C_RED
      }
    ...
    changeEntityPerformance: function () {
      ...
      catFromTo = "";
      if (Game.isPerformanceRoundWith12) {
        console.log(Game.scriptShortVersion[Game.performanceTrial])
        catFromTo = Game.scriptShortVersion[Game.performanceTrial].split("|")
      } else {

        catFromTo = Game.scriptLongVersion[Game.performanceTrial].split("|")
        console.log(Game.scriptLongVersion[Game.performanceTrial])
      }
    ...
    createEntitiesRatio: function () {
      ...
      //forzar borrar los actuales

      table = {}


      // console.log(" %c Conditions ", 'background: #ff0000')
      if (Game.performanceFirstRound && !Game.performanceSecondRoundRound) {
        if ((Game.idParticipant % 2) == 0) { //starts with twelve
          console.log(" %c  participant  %d should start with 12 ", 'background: #ff00dd', Game.idParticipant)
        }
        if ((Game.idParticipant % 2) != 0) { //starts with 24
          console.log(" %c  participant  %d should start with 24 ", 'background: #ff00dd', Game.idParticipant)
        }
      }

      if (Game.performanceFirstRound && Game.performanceSecondRoundRound) {
        if ((Game.idParticipant % 2) == 0) { //starts with twelve
          console.log(" %c  participant  %d should finish with 24 ", Game.idParticipant, 'background: #ff00dd')
        }
        if ((Game.idParticipant % 2) != 0) { //starts with 24
          console.log(" %c  participant  %d should finish with 12 ", 'background: #ff00dd', Game.idParticipant)
        }
      }

      if (!Game.performanceFirstRound && Game.performanceSecondRoundRound) {
        console.log(" %c  participant  %d satate weird ", Game.idParticipant, 'background: #ff00dd')
    ...
    if (Game.conditionExperiment === 1 || Game.conditionExperiment === 4) {
      if (Game.isPerformanceRoundWith12) {
        table = TrialsRatio.entitiesCond1_12;
      } else {
        table = TrialsRatio.entitiesCond1_24;
      }
    } else {
      if (Game.isPerformanceRoundWith12) {
        table = TrialsRatio.entitiesCond2_12;
      } else {
        table = TrialsRatio.entitiesCond2_24;
      }
    }
  ...
  if (Game.inTraining) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        // Game.isPerformanceRoundWith12 = Game.idParticipant % 2 == 0;
        intracondition = Game.isPerformanceRoundWith12 ? "without" : "with";//12 or 24
        console.log(" PERFOMANCE  case with 12 entities? ", Game.isPerformanceRoundWith12);
              ...
              [
                {
                  ...
                  'Now, for the <b>main monitoring task</b>.<br/> Here, you are' +
                  ' in an area of ocean <b>' + intracondition + '</b> many craft.<br/><br/> Remember, you need to click on a craft when you\n' +
    ...
    if (Game.performanceFirstRound && !Game.performanceSecondRound) {
      Game.layerSwitchTo("GameStart");
      ...
      $.ajax({
        ...
        data: {
          ...
          replay: JSON.stringify(Game.replay),
          // first_round_12: Game.isPerformanceRoundWith12 ? 1 : 0
        },

        success: function (data) {
          Game.layerSwitchTo("GameStart");
          // Game.isPerformanceRoundWith12 = !Game.isPerformanceRoundWith12;
          console.log('%c Starting second roundPerformance session! SUCCESS', 'background: #aaaacc; color: 99ffdd');
          //                        console.log("Saved " + data);
          console.log(" PERFOMANCE  case with 12? ", Game.isPerformanceRoundWith12);
          intracondition = Game.isPerformanceRoundWith12 ? "without" : "with";//12 or 24
                ...
                [
                  {
                    ...
                    type: "html",
                    name: "Summary",
                    html: '<h1> Monitoring Task</h1>' + 'Now, for the <b>main monitoring task</b>.<br/> Here, you are' +
                    ' in an area of ocean  <b>' + intracondition + '</b> many craft.<br/><br/> Remember, you need to click on a craft when you\n' +
                    'notice that it changes status.<br/><br/>  <br/><b>Do not to switch between browser tabs or your participation could be voided.</b>',
      ...
      //  console.log("Regresando de performance first round y second roundion")
      if (Game.performanceFirstRound && Game.performanceSecondRound) {
        Game.inPerformance = false;
        console.log('%c Finishing Performance session 2nd part! ', 'background: #aaaacc; color: 99ffdd');
        $.ajax({
          type: "POST",
